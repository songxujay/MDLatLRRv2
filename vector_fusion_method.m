...
%----------输出----------
%F:融合的灰度图像结果
function F = vector_fusion_method(img1_or, img2_or, L, P, unit, de_level, norm, is_overlap, stride)
[t1,t2] = size(img1_or);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% is_overlap = 0;
[I_b1,I_d1_deep, show_matrix1, count1, I_vector1] = vector_deep_latent(img1, de_level, L, P, unit, is_overlap, stride, w_num);
[I_b2,I_d2_deep, show_matrix2, count2, I_vector2] = vector_deep_latent(img2, de_level, L, P, unit, is_overlap, stride, w_num);
count_all = count1;
% fuion for base parts，same size as the original
...
if is_overlap==1
    ...
    for i=1:de_level
        temp1 = I_d1_deep(:,:,i);
        temp2 = I_d2_deep(:,:,i);
        [temp_vector] = vector_fuison_detail_parts(temp1, temp2, norm);
        c1 = 0;
        for ii=1:s:h-unit+1
            c2 = 0;
            c1 = c1+1;
    ...
    I_df_vector = vector_fuison_detail_parts(temp1, temp2, norm);
